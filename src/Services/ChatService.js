...
export function useGetGlobalMessages() {
  ...
  const getGlobalMessages = () => {
    return fetch(
      `${'http://localhost:3007'}/api/messages/global`,
      requestOptions
    )
      .then(handleResponse)
      .catch((e) => e);
...
export function useGetConversationMessages() {
  ...
  const getConversationMessages = (id) => {
    return fetch(
      `${'http://localhost:3007'}/api/messages/conversations/query?userId=${id}`,
      requestOptions
    )
      .then(handleResponse)
      .catch((e) => e);
...
export function useSendConversationMessage() {
  ...
  const sendConversationMessage = (id, body) => {
    ...
    return fetch(`${'http://localhost:3007'}/api/messages/`, requestOptions)
      .then(handleResponse)
      .catch((e) => e);
  };
